    ...
    : ITweener
{
    ...
    /// The tweens.
    /// </summary>
    private readonly Dictionary<object, List<ITweenable>> tweens;

    /// <summary>
    /// The to remove.
    /// </summary>
    private readonly List<ITweenable> toRemove;

    /// <summary>
    /// The to add.
    /// </summary>
    private readonly List<ITweenable> toAdd;

    /// <summary>
    /// The all tweens.
    /// </summary>
    private readonly List<ITweenable> allTweens;
    #endregion Fields
    ...
    static Tweener()
    {
        ...
        RegisterLerper<PointFLerper>(typeof(PointF));
        RegisterLerper<SizeFLerper>(typeof(SizeF));
        RegisterLerper<Vector2Lerper>(typeof(Vector2));
        RegisterLerper<Vector3Lerper>(typeof(Vector3));
    }
    ...
    public Tweener()
    {
        tweens = new Dictionary<object, List<ITweenable>>();
        toRemove = new List<ITweenable>();
        toAdd = new List<ITweenable>();
        allTweens = new List<ITweenable>();
    }
    ...
    public ITweenable Tween<T>(T target, object dests, float duration, float delay = 0, bool overwrite = true) where T : class
    {
        if (target is null)
        {
    ...
    public ITweenable Timer(float duration, float delay = 0)
    {
    ...
    public void Update(float ticksElapsed)
    {
        foreach (var tween in allTweens)
        {
    ...
    internal void Remove(ITweenable tween) => toRemove.Add(tween);
    ...
    private void AddAndRemove()
    {
        foreach (var tween in toAdd)
        {
            ...
            {
                tweens[tween.Target] = list = new List<ITweenable>();
            }
